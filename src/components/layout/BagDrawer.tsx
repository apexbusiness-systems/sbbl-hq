...
export const BagDrawer = () => {
  ...
  const handleCheckout = async () => {
    ...
    // ⚡ Bolt Performance Optimization: Replace O(N^2) array finds with O(1) dictionary lookups
    const groupedItems = bagItems.reduce<Record<string, { id: string; name: string; price: number; qty: number }>>((acc, id) => {
      const product = productMap[id];
    ...
    const lineItems = Object.values(groupedItems);

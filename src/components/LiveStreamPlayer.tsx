...
export function LiveStreamPlayer({
  ...
  // Memoize host classification so downstream overlays only re-evaluate on URL changes.
  const isFacebookStream = useMemo(
    () => /facebook\.com|fb\.watch/i.test(playbackUrl),
    [playbackUrl],
  );
  ...
  return (
    <div className="absolute inset-0">
      ...
      {/* Keep session provenance visible for authenticated viewers, even while
          gated, so entitlement/evidence tests can assert the watermark. */}
      <div className="absolute top-4 right-4 text-[10px] text-white/10 font-mono pointer-events-none select-none">
        SESSION-BOUND · {userId.slice(0, 8).toUpperCase()}
      </div>
    </div>

...
async function handleImportRoute(ctx: HandlerCtx, kind: 'teams' | 'players' | 'schedules' | 'events') {
  ...
  try {
    if (kind === 'teams') {
      ...
      if (error) throw error; // Any error, including duplicates, triggers the fallback to process partial successes
    ...
    // Call the RPC for each row concurrently to maintain proper event outbox semantics
    await Promise.all(
      rows.map(row =>
    ...
    // If the bulk batch fails, fall back to individual inserts to preserve partial success and exact error logging
    for (const row of rows) {
      try {
        if (kind === 'teams') {
          const { error: err } = await ctx.admin.from('teams').insert({
            league_id: row.league_id,
            season_id: row.season_id,
            division_id: row.division_id || null,
            name: row.name,
          ...
          if (err && !String(err.message).includes('duplicate key')) throw err;
        }

        if (kind === 'players') {
          const { error: err } = await ctx.admin.from('players').upsert({
            user_id: row.user_id,
            team_id: row.team_id || null,
            league_id: row.league_id || null,
            jersey_number: row.jersey_number ? Number(row.jersey_number) : null,
        ...
        if (kind === 'schedules') {
          const { error: err } = await ctx.admin.from('schedule_slots').insert({
            league_id: row.league_id,
            season_id: row.season_id,
            venue_id: row.venue_id || null,
            court_id: row.court_id || null,
        ...
        if (kind === 'events') {
          const { error: err } = await ctx.admin.from('league_events').insert({
            league_id: row.league_id || null,
            season_id: row.season_id || null,
            venue_id: row.venue_id || null,
            title: row.title,
...
async function handlePublicHome({ req, admin }: HandlerCtx) {
  ...
  const [leaguesRes, teamsRes, gamesRes, seasonsRes] = await Promise.all([
    admin.from('leagues').select('id,name,code').order('name'),
    admin.from('teams')
      .select('id,name,leagues(name,code),seasons(name),divisions(name),players(id)')
      .eq('status', 'published')
      .limit(200),
    admin.from('games')
...
async function handleTeamsList({ req, admin }: HandlerCtx) {
  ...
  let gamesQuery = admin.from('games')
    .select('id,home_team_id,away_team_id,status,home_score,away_score,league_id,seasons(leagues(code))')
    .eq('status', 'final');

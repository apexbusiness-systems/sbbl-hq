...
export async function fetchAdminStreamConfig(token: string | null) {
  // Guard: do not fire an authenticated ops route without a valid token.
  // Callers may invoke this before the session is ready (e.g. during login
  // flow). Without this guard, the Worker returns 401 which previously
  // triggered a forced sign-out via apiFetch's clearLocalAuthState().

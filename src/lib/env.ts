...
const clientEnvSchema = z.object({
  ...
  // Every flag defaults off. Flipping from 'false' → 'true' is a
  // forward-only roll; there is no bake-in state that requires a flag
  // to stay on. Safe to turn off at any time.
  VITE_FEATURE_SHOW_VIEWER_PREFLIGHT: z.enum(['true', 'false']).default('false'),
  VITE_FEATURE_FAN_TOKEN_SYSTEM: z.enum(['true', 'false']).default('false'),
  VITE_FEATURE_BIOMETRIC_OVERLAY: z.enum(['true', 'false']).default('false'),
  VITE_FEATURE_MIC_UP_SERIES: z.enum(['true', 'false']).default('false'),
});

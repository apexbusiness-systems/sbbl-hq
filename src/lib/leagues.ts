...
/** ⚡ Bolt Performance Optimization: Precomputed O(1) lookup structures */
export const VALID_LEAGUE_IDS = new Set<string>(LEAGUE_REGISTRY.map((l) => l.id));
export const LEAGUE_ID_MAP = new Map<string, LeagueIdentity>(LEAGUE_REGISTRY.map((l) => [l.id, l]));
export const LEAGUE_CODE_MAP = new Map<string, LeagueIdentity>(LEAGUE_REGISTRY.map((l) => [l.code, l]));

export function getLeagueConfig(id: LeagueId): LeagueIdentity {
  return LEAGUE_ID_MAP.get(id) ?? LEAGUE_ID_MAP.get('sbbl')!;
}

export function leagueIdFromCode(code: string): LeagueId {
  const upper = code.toUpperCase();
  const match = LEAGUE_CODE_MAP.get(upper);
...
export function loadPersistedLeague(): LeagueId | null {
  try {
    ...
    if (stored && VALID_LEAGUE_IDS.has(stored)) return stored as LeagueId;
  } catch { /* noop */ }
  return null;
}

...
// ⚡ Bolt Performance Optimization: Precompute O(1) lookup structures for league config
export const LEAGUE_MAP = new Map<LeagueId, LeagueIdentity>(
  LEAGUE_REGISTRY.map((l) => [l.id, l])
);

export const LEAGUE_ID_SET = new Set<LeagueId>(
  LEAGUE_REGISTRY.map((l) => l.id)
);

const LEAGUE_CODE_MAP = new Map<string, LeagueId>(
  LEAGUE_REGISTRY.map((l) => [l.code, l.id])
);
...
export function leagueIdFromCode(code: string): LeagueId {
  ...
  return LEAGUE_CODE_MAP.get(upper) ?? 'sbbl';
}

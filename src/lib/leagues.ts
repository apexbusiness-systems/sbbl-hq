...
export const VALID_LEAGUE_IDS = new Set(LEAGUE_REGISTRY.map((l) => l.id));
export const LEAGUE_REGISTRY_MAP = new Map(LEAGUE_REGISTRY.map((l) => [l.id, l]));
export const LEAGUE_CODE_MAP = new Map(LEAGUE_REGISTRY.map((l) => [l.code, l]));

export function getLeagueConfig(id: LeagueId): LeagueIdentity {
  return LEAGUE_REGISTRY_MAP.get(id) ?? LEAGUE_REGISTRY_MAP.get('sbbl')!;
}
...
export function loadPersistedLeague(): LeagueId | null {
  try {
    ...
    if (stored && VALID_LEAGUE_IDS.has(stored as LeagueId)) return stored as LeagueId;
  } catch { /* noop */ }
  return null;
}

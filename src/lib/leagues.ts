...
// ⚡ Bolt: O(1) Set lookup for fast league validation instead of O(N) Array.some()
export const LEAGUE_ID_SET = new Set(LEAGUE_REGISTRY.map(l => l.id));
// ⚡ Bolt: O(1) Map lookup for fast league config retrieval instead of O(N) Array.find()
export const LEAGUE_MAP = new Map(LEAGUE_REGISTRY.map(l => [l.id, l]));

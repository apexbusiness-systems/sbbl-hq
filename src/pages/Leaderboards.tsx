...
import { LEAGUE_REGISTRY, VALID_LEAGUE_IDS, LEAGUE_REGISTRY_MAP } from '@/lib/leagues';
...
const LeaderboardsPage = () => {
  ...
  const initialFilter: LeagueId | 'all' =
    paramLeague && (paramLeague === 'all' || VALID_LEAGUE_IDS.has(paramLeague as LeagueId))
  ...
  const isValidParam = paramLeague && (paramLeague === 'all' || VALID_LEAGUE_IDS.has(paramLeague as LeagueId));
  ...
  const activeLeagueObj = leagueFilter === 'all' ? null : (LEAGUE_REGISTRY_MAP.get(leagueFilter as LeagueId) ?? null);
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              {visible.slice(0, 3).map((p, i) => (
                <div key={p.id} className={`panel p-4 text-center ${i === 0 ? 'border-primary/30' : ''}`}>
                  ...
                  <PlayerAvatar src={p.avatar || null} alt={p.name} className="w-16 h-16 mx-auto mb-2" />
              ...
              {visible.map((p, i) => {
                ...
                return (
                  <div key={p.id} className={`panel p-3 flex items-center gap-4 ${i < 3 ? 'border-primary/20' : ''}`}>
                    ...
                    <PlayerAvatar src={p.avatar || null} alt={p.name} className="w-10 h-10" />

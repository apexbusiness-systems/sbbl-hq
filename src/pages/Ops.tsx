...
function OpsCsvImportSection({ kind, csvUpload, csvLeagueId, setCsvLeagueId, isSuperAdmin }: OpsCsvImportSectionProps) {
  ...
  // ⚡ Bolt Performance Optimization: Deduplicate inline filter
  // Extract queue filtering to a local variable to prevent redundant O(N) evaluations
  // across multiple references in the render cycle.
  const filteredQueue = csvUpload.queue.filter(q => q.type === kind);

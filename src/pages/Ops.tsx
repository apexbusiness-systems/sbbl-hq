...
const OpsPage = () => {
  ...
  const scoresCsvMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['ops-scores-list'] }),
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="panel p-4 md:col-span-3">
            ...
            {latestSummary.length === 0 ? (
              ...
              latestSummary.map((job) => (
                <p key={job.id} className="text-sm">
                  {job.job_type} · {job.status} · {job.inserted_rows}/{job.total_rows}
                </p>
              ))
            )}
          </div>
      ...
      {activeTab === 'streams' && (
        <div className="space-y-4">
          ...
          <div className="panel p-4 space-y-3 max-w-2xl">
            ...
            <div>
              <label htmlFor="collectionId" className="sr-only">Switcher collection ID</label>
              <input
                id="collectionId"
            ...
            <div>
              <label htmlFor="streamTitle" className="sr-only">Stream title</label>
              <input
                id="streamTitle"
        ...
        <div className="space-y-6">
          ...
          <div className="panel p-4 space-y-4 max-w-2xl">
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label="Upload scoreboard image"
              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') scoreboardFileRef.current?.click(); }}
            >
              <label htmlFor="scoreboard-upload" className="sr-only">Scoreboard photo upload</label>
              <input id="scoreboard-upload" ref={scoreboardFileRef} type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) void handleScoreboardImage(f); }} />
              ...
              {scoreboardParseState === 'parsed' && (
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-[10px] text-muted-foreground">Click or drop to parse another image</p>
                </div>
              )}
              {scoreboardParseState === 'error' && (
                <div className="flex flex-col items-center gap-1">
                  <AlertCircle className="w-5 h-5 text-destructive" />
                  <p className="text-xs text-destructive">{scoreboardParseError}</p>
                  <p className="text-[10px] text-muted-foreground">Fill in fields manually below</p>
                </div>
              )}
              {scoreboardParseState === 'idle' && (
          ...
          <div className="panel p-4 space-y-4 max-w-2xl">
              ...
              <div>
                ...
                <div className="flex gap-1 p-1 bg-secondary rounded-sm w-fit">
                  {LEAGUE_REGISTRY.map(l => {
                    const isActive = scoresForm.leagueId === l.id;
                    return (
                      <button 
                        key={l.id} 
                        type="button" 
                        onClick={() => setScoresForm(f => ({ ...f, leagueId: l.id }))}
                        className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold uppercase tracking-wider rounded-sm transition-colors ${isActive ? `bg-card ${l.accentClass} border border-current/20` : 'text-muted-foreground hover:text-foreground'}`}
                      >
            ...
            {scoreManualMutation.isSuccess && (
              <p className="text-xs text-success">
                Score saved — game ID: {scoreManualMutation.data?.gameId?.slice(0, 8)}
              </p>
            )}
          </div>
          ...
          <div className="panel p-4 space-y-3 max-w-2xl">
            ...
            <div>
              <label htmlFor="csv-upload" className="sr-only">CSV file upload</label>
              <input id="csv-upload" ref={scoresCsvFileRef} type="file" accept=".csv,text/csv" onChange={async e => {
              ...
              {(scoresQuery.data?.games ?? []).slice(0, 20).map(g => (
                <div key={g.id} className="border border-border rounded-sm p-3 text-xs flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`flex-shrink-0 px-1.5 py-0.5 rounded-sm text-[10px] font-bold uppercase ${
                      g.category === 'league' 
                      ...
                      {g.category === '1v1' 
                        ? '1v1' 
                        : g.category === 'special_event' 
                          ? 'Event' 
                          : (g.leagueCode ?? g.leagueId ?? 'LGE').toUpperCase()
                      }
                    </span>

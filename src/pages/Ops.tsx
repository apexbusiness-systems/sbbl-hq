...
function OpsCsvImportSection({ kind, csvUpload, csvLeagueId, setCsvLeagueId, isSuperAdmin }: OpsCsvImportSectionProps) {
  ...
  // ⚡ Bolt Performance Optimization: Extract repeated O(N) array filtering to useMemo to avoid recalculation on every render
  const filteredQueue = useMemo(() => {
    return csvUpload.queue.filter(q => q.type === kind);

...
import { LEAGUE_REGISTRY, getLeagueConfig, getLeagueSeasonLabel, VALID_LEAGUE_IDS } from '@/lib/leagues';
...
const SchedulesPage = () => {
  ...
  const mappedLiveSchedules: ScheduleDay[] = useMemo(() => {
    ...
    const groupedLive = liveSchedules.reduce<Record<string, Omit<ScheduleDay, 'courts'> & { courts: Record<string, ScheduleGame[]> }>>((acc, curr) => {
      ...
      if (!acc[key]) {
        const leagueId = VALID_LEAGUE_IDS.has(rawLeagueId as LeagueId)

...
## 2024-05-18 - [Conditional Component Hooks Extraction]
**Learning:** Placing hooks in components that are conditionally hidden via early returns (e.g., `if (!isOpen) return null;`) causes React to execute those hooks (like expensive `useMemo` operations or `useQuery` fetches) in the background even when the component is not visible, degrading performance.
**Action:** Extract expensive logic and hooks into an inner component (e.g., `BagDrawerContent`) that is conditionally rendered by the parent (e.g., `if (!bagOpen) return null; return <BagDrawerContent />;`), ensuring hooks only execute when the UI is actively displayed.

...
## 2025-05-14 - [Performance] Optimized League Registry Lookups
**Learning:** In React components and utility functions, calling `.find()` or `.some()` on the static `LEAGUE_REGISTRY` array creates an $O(N)$ lookup on every render cycle or function call.
**Action:** Replace $O(N)$ array lookups with $O(1)$ dictionary lookups by exporting a `LEAGUE_MAP` and `LEAGUE_ID_SET` alongside `LEAGUE_REGISTRY`, and utilizing them across the codebase to ensure optimal performance.

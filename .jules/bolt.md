...
**Learning:** Found array `.filter()` calculations running three separate times inline within a React component's rendering flow to conditionally display lengths and map items, which triggers repeated O(N) operations on every single re-render.
**Action:** Extract repeated O(N) functional loops (like `.filter`) from directly inside returned TSX elements into `useMemo` hooks above the render, especially when multiple references are needed for condition checks, lengths, and mapping.

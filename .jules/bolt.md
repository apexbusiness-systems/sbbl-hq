...
## 2026-09-09 - [Performance] Optimized Schedule Grouping Loop\n**Learning:** Using array search methods like `.some()` inside a React `useMemo` `.reduce()` loop over a large dataset creates an O(N * M) performance bottleneck that runs whenever dependencies change.\n**Action:** To optimize React renders involving static configuration lists (e.g., `LEAGUE_REGISTRY`), precompute O(1) lookup structures like `Set` or `Map` at the module scope (outside the component) rather than using O(M) array methods inside render loops or `useMemo` dependencies.

...
## 2026-05-24 - [Performance] Optimized Static Configuration Lookups
**Learning:** Found static configuration arrays (like `LEAGUE_REGISTRY`) being searched via `Array.prototype.find()` on every render cycle in `src/pages/Stats.tsx`. Since the data is static, this incurs unnecessary O(N) overhead.
**Action:** Precompute O(1) lookup structures like `Map` or `Set` alongside static data exports and use them directly, preventing repeated array traversals across the application.

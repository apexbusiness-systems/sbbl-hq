...
## 2026-09-26 - [Performance] Optimized League Registry Lookups
**Learning:** Using array methods like `.find()` on `LEAGUE_REGISTRY` inside React render paths causes O(N) operations on every render.
**Action:** Precompute O(1) structures like `LEAGUE_MAP` and `LEAGUE_ID_SET` at module scope to eliminate unnecessary CPU overhead.

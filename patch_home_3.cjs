...
const regex2 = /\{\/\* Players of the Game — always rendered; empty state when no data for this league yet \*\/\}\s*\{\(\(\) => \{\s*const potgList = playersOfTheGame\.filter\(p => p\.leagueId === resolvedLeague\);\s*const leagueInfo = getLeagueConfig\(resolvedLeague\);\s*return \(\s*<section>[\s\S]*?<\/section>\s*\);\s*\}\)\(\)\}/g;
...
code = code.replace(regex2, newPotgBlock);

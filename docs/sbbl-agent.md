...
This repository now includes a local `sbbl-agent` skill artifact so the session-level
instruction can be tracked in-repo and referenced by contributors.
...
- Preserve brand and operating constraints (dark-gold design, realtime-first,
  RLS-on-every-table, test-before-merge).

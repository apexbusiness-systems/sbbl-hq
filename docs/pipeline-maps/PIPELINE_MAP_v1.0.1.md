...
* **AI Image Parsing (OCR & Vision):**
  * `parseScoreboardImage` (`/ops/scores/parse-image`): Sends base64-encoded images to the Groq vision API and extracts `homeLabel`, `awayLabel`, `homeScore`, `awayScore`, `gameDate`, `eventName`, and `status`.
...
- Evidence Bundle: `docs/quality/evidence/*_2026-04-07.*`
- Evidence digests: included in the signed QA matrix (SHA-256 table)
- Validation Outcome: full worker route checklist PASS (90/90), ingress/render checklist PASS (40/40), parser checklist PASS, full regression PASS.

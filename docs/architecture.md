...
2. Worker resolves user session from bearer token (`supabase.auth.getUser`) or fallback headers.
3. Route matching is done from explicit route table (`method + regex` compiled from static path patterns).
4. Handlers call either:
...
5. Unmatched requests pass to static asset handler (`ASSETS`) and then return 404 JSON for missing routes.

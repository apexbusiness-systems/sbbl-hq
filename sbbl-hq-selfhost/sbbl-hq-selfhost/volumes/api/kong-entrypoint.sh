...
# When opaque keys are not configured, expressions fall through to legacy API keys.
...
# Substitute env vars without eval so YAML quoting is preserved.
awk '{
  result = ""
  rest = $0
...
# Remove empty key-auth credentials for unconfigured opaque keys.
